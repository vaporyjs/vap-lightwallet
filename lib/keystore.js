...
KeyStore.prototype.signTransaction = function (txParams, callback) {
  ...
  vapjsTxParams.from = add0x(txParams.from);
  vapjsTxParams.to = add0x(txParams.to);
  vapjsTxParams.gasLimit = add0x(txParams.gas);
  vapjsTxParams.gasPrice = add0x(txParams.gasPrice);
  vapjsTxParams.nonce = add0x(txParams.nonce);
  vapjsTxParams.value = add0x(txParams.value);
  vapjsTxParams.data = add0x(txParams.data);

  var txObj = new Transaction(vapjsTxParams);
